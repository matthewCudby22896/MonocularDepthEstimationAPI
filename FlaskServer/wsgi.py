    ...
    if model == MARIGOLD:
        if denoising_steps is None:
            ...
            try:
                ...
                logger.error("Invalid 'denoising_steps': must be an int")
        ...
        if ensemble_size is None:
            ...
            try:
                ...
                logger.error("Invalid 'ensemble_size': must be an int")
...
def model_inference(img_bgr : ImageNP,
    ...
    assert depth_map.shape[0:2] == img_bgr.shape[0:2]
    assert confidence is None or confidence.shape == img_bgr.shape[0:2]
